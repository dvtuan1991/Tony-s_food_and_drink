...
const ProductAdminForm: FC<{ product?: IProduct, isCreate?: boolean }> = ({ product, isCreate }) => {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      const isImageType = files[0].type === 'image/jpeg'
  ...
  const getCategoryData = useCallback(async () => {
    const responseListCategory: ICategory[] = await fetchApi(`${SERVICE_API}/category`);
                ...
                <Row justify="center" gutter={16}>
                  ...
                  <Col span={12}  >
                    <div className={`${styles["input-box"]} mb-4`}>
                      <Form.Item name='name' label='Food Name' rules={[{ required: true, max: 50 }]}>
                        <Input placeholder="Food Name" />
                      </Form.Item>
                      {listCategory &&
                        <Form.Item name='categoryId' label="Category Name" rules={[{ required: true }]}>

                          <Select allowClear={true} >
                            {listCategory.map(category => (
                              <Option key={category.id} value={category.id} label={category.name.toLocaleUpperCase()} >
                                {category.name}
                              </Option>))}
                          </Select>
                        </Form.Item>}
